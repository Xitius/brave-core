...
// Tests that a null WebState causes the prompt not to be handled.
TEST_F(PromptFacadeTest, NullWebStateReturnsNullopt) {
  std::unique_ptr<PromptFacade> handler =
      std::make_unique<web::PromptFacade>(nullptr);

  std::string prompt = BuildPromptJSON(kHandlerName, base::DictValue());
  EXPECT_EQ(std::nullopt,
            handler->HandleJavaScriptPrompt(GURL("https://example.com/"),
                                            /*is_main_frame=*/true, prompt));
}
