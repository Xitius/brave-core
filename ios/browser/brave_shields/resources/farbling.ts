...
function enableFarbling(args: FarblingArgs) {
  ...
  const farbleAudio = (fudgeFactor: number) => {
    ...
    for (const [structure, methodName] of structuresToFarble) {
      ...
      proto[methodName as string] = function() {
  ...
  const farblePlugins = (fakePluginData: PluginData[]) => {
    ...
    if (window.navigator.plugins !== undefined) {
      ...
      pluginsPrototype['item'] = function(index: number) {
        if (index < originalPluginsLength) {
          return Reflect.apply(
            originalItem as Function,
            this,
            arguments
          ) as Plugin
        } else {
      ...
      pluginsPrototype['namedItem'] = function(name: string) {
        let namedPlugin = Reflect.apply(
          originalNamedItem as Function,
          this,
          arguments
        ) as Plugin | null
  ...
  const farbleVoices = (
    ...
    const getVoicesPrototype: Record<string, Function> =
      Object.getPrototypeOf(window.speechSynthesis)
    getVoicesPrototype['getVoices'] = function() {
      const voices = Reflect.apply(getVoices, this, arguments) as
        SpeechSynthesisVoice[]

      if (fakeVoice === undefined) {
        const randomVoiceIndex =
          Math.round(randomVoiceIndexScale * voices.length)
