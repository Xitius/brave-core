...
LoggedOutState::LoggedOutState(
    ...
    : StateBase(account_state_prefs, std::move(url_loader_factory), encryptor) {
}

LoggedOutState::~LoggedOutState() = default;

void LoggedOutState::RegisterInitialize(
...
void LoggedOutState::RegisterVerify(
...
void LoggedOutState::LoginInitialize(
    mojom::Service initiating_service,
    const std::string& email,
...
void LoggedOutState::LoginFinalize(
...
void LoggedOutState::OnRegisterInitialize(
    mojom::Authentication::RegisterInitializeCallback callback,
...
void LoggedOutState::OnRegisterFinalize(
  ...
  // Update the pref state before invoking the Mojo callback,
  // so observers and the callback chain see the updated pref state.
  if (result.has_value()) {
...
void LoggedOutState::OnRegisterVerify(
    mojom::Authentication::RegisterVerifyCallback callback,
  ...
  // Trigger the state transition before invoking the Mojo callback,
  // so observers and the callback chain see the updated pref state.
  if (result.has_value()) {
    CHECK(!email.empty());
    ...
    // SetLoggedIn() re-enters via OnAccountStateChanged() and destroys `this`;
...
void LoggedOutState::OnLoginInitialize(
    mojom::Authentication::LoginInitializeCallback callback,
...
void LoggedOutState::OnLoginFinalize(
    mojom::Authentication::LoginFinalizeCallback callback,
