...
# prefix to the directory that archive lives under in the clang bucket, mirroring
# the `host_os` list in `tools/clang/scripts/sync_deps.py:GetRustObjectNames`.
...
def rust_toolchain_extra_dep() -> dict[str, dict]:
    ...
    """
    tools_rust = str(Path(__file__).resolve().parents[4] / TOOLS_RUST)
    if tools_rust not in sys.path:
        sys.path.insert(0, tools_rust)
    import package_rust  # pylint: disable=import-outside-toplevel
    upstream_stem = package_rust.RUST_TOOLCHAIN_PACKAGE_NAME.removesuffix(
        '.tar.xz')
